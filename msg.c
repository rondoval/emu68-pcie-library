...
int bcm2835_power_on_module(ULONG module)
{
	...
	ret = bcm2835_mbox_call_prop(BCM2835_MBOX_PROP_CHAN,
				     &msg_pwr->hdr);
	if (ret) {
		Kprintf("[mailbox] Could not set module %lu power state\n",
		       module);
		return -EIO;
	}
...
int bcm2835_get_mmc_clock(ULONG clock_id)
{
	...
	if (ret) {
		Kprintf("[mailbox] Could not query eMMC clock rate\n");
		return -EIO;
	}
	...
	if (clock_rate == 0) {
		...
		ret = bcm2835_mbox_call_prop(BCM2835_MBOX_PROP_CHAN, &msg_clk->hdr);
		if (ret) {
			Kprintf("[mailbox] Could not query max eMMC clock rate\n");
			return -EIO;
		}
...
int bcm2835_set_sdhost_clock(ULONG rate_hz, ULONG *rate_1, ULONG *rate_2)
{
	...
	if (ret) {
		Kprintf("[mailbox] Could not query sdhost clock rate\n");
		return -EIO;
	}
...
int bcm2835_get_video_size(int *widthp, int *heightp)
{
	...
	if (ret) {
		Kprintf("[mailbox] Could not query display resolution\n");
		return ret;
	}
			     ...
			     ULONG *fb_sizep, int *pitchp)
{
	...
	if (ret) {
		Kprintf("[mailbox] Could not configure display\n");
		return ret;
	}
	...
	*fb_basep = msg_setup->allocate_buffer.body.resp.fb_address;
